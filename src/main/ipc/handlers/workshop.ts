...
export const workshopHandlers: IpcHandlersFor<typeof workshopRequestSchemas> = {
  ...
  'workshop.prompt.list': async (input) =>
    exposeWorkshopError(() => application.get('WorkshopService').listPrompts(input)),
  ...
  'workshop.discussion.list': async (input) =>
    exposeWorkshopError(() => application.get('WorkshopService').listDiscussion(input)),
  ...
  'workshop.volume.start': async (input) =>
    exposeWorkshopError(() => application.get('WorkshopService').startVolumeRun(input)),
  'workshop.export': async (input) =>
    exposeWorkshopError(() => application.get('WorkshopService').exportManuscript(input))
}
