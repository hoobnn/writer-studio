...
export class AppService extends BaseService {
  ...
  public async setAppLaunchOnBoot(isLaunchOnBoot: boolean): Promise<void> {
    ...
    if (isWin || isMac) {
      ...
      const desktopFile = AbsoluteFilePathSchema.parse(
        path.join(autostartDir, isDev ? `${DISTRIBUTION.protocol}-dev.desktop` : `${DISTRIBUTION.protocol}.desktop`)
      )
  ...
  Comment=Professional AI writing studio.
  Exec=${executablePath}
  Icon=${DISTRIBUTION.protocol}
  Terminal=false
  StartupNotify=false
